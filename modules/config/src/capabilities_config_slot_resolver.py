"""Config capability (capabilities_config_slot_resolver): resolve slot inputs to AppConfig.
...
class SlotRunPlanResolver(ISlotRunPlanProtocol):
    ...
    def discover_batch_prompts(self, batch_dir: FilePath) -> BatchPromptOutcome:
        ...
        Returns a BatchPromptOutcome carrying an error message when the
        directory is missing or empty, so the surface layer only forwards
        the message.
        """
        raw_dir = str(batch_dir)
        if not raw_dir.strip():
            return BatchPromptOutcome(error=SlotInputValue("Please specify a directory path."))
        batch_path = Path(raw_dir).expanduser().resolve()
        if not batch_path.is_dir():
            return BatchPromptOutcome(error=SlotInputValue(f"Directory not found: {batch_path}"))
